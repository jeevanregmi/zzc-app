...
function formatNPR(n: number): string {
  ...
  if (n >= 100_000) return `NPR ${(n / 100_000).toFixed(2)} L`;
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl px-4 py-3 text-sm">
        ...
        <p className="text-green-400">Fund: {formatNPR(payload[0]?.value)}</p>
        <p className="text-zinc-400">Contributed: {formatNPR(payload[1]?.value)}</p>
      </div>
    );
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 mb-10">
          ...
          <div className="mb-6">
            ...
            <label className="block font-bold text-lg mb-2">Scheme</label>

            {loading ? (
              <p className="text-zinc-600 text-sm">Loading schemes...</p>
            ) : (
              <select
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block font-bold mb-2">Monthly Contribution (NPR)</label>
              <input
                type="number"
                min={100}
                step={500}
            ...
            {/* Annual Rate */}
            <div>
              <label className="block font-bold mb-2">Annual Interest Rate (%)</label>
              <input
                type="number"
                min={0}
                max={30}
            ...
            {/* Current Age */}
            <div>
              <label className="block font-bold mb-2">Current Age</label>
              <input
                type="number"
                min={15}
                max={59}
            ...
            {/* Retirement Age */}
            <div>
              <label className="block font-bold mb-2">Retirement Age</label>
              <input
                type="number"
                min={40}
                max={70}
        ...
        {projectionData.length > 0 && (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
              <p className="text-zinc-500 text-sm mb-1">Fund at Retirement</p>
              <p className="text-green-400 font-black text-xl">{formatNPR(totalFund)}</p>
            </div>

            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
              <p className="text-zinc-500 text-sm mb-1">Total Contributed</p>
              <p className="text-blue-400 font-black text-xl">{formatNPR(totalContributed)}</p>
            </div>

            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
              <p className="text-zinc-500 text-sm mb-1">Interest Earned</p>
              <p className="text-purple-400 font-black text-xl">{formatNPR(totalInterest)}</p>
            </div>

            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
              <p className="text-zinc-500 text-sm mb-1">Est. Monthly Pension</p>
              <p className="text-orange-400 font-black text-xl">{formatNPR(monthlyPension)}</p>
              <p className="text-zinc-700 text-xs mt-1">over {yearsInRetirement} yrs</p>
            </div>
        ...
        {projectionData.length > 0 && (
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8">
            ...
            <h2 className="text-2xl font-black mb-2">Fund Growth</h2>

            <p className="text-zinc-500 text-sm mb-6">
              Age {currentAge} → {retirementAge} · {retirementAge - currentAge} years · {annualRate}% p.a.
            </p>
            ...
            <ResponsiveContainer width="100%" height={320}>
              ...
              >
                ...
                <XAxis
                  ...
                  label={{ value: "Age", position: "insideBottom", offset: -2, fill: "#52525b" }}
                />
                <YAxis
            ...
            <div className="flex gap-6 mt-4 text-sm">
              <span className="flex items-center gap-2 text-zinc-400">
                <span className="w-4 h-0.5 bg-green-400 inline-block" /> Total Fund
              </span>
              <span className="flex items-center gap-2 text-zinc-400">
                <span className="w-4 h-0.5 bg-blue-500 inline-block border-t border-dashed" /> Amount Contributed
              </span>
            </div>
        ...
        {projectionData.length === 0 && !loading && (
          <div className="text-center text-zinc-600 py-16">
            <p className="text-xl font-bold">Set your contribution and age range above</p>
          </div>
        )}
